...
class TransformerTextModelMultiLossTokenClassification:
    ...
    def predict(
        ...
        eval_ce_losses, logits, out_label_ids, true_seg_boundaries, batch_keys, originalword2subtokens = zip(*list(maybe_tqdm(
            map(
                partial(
        ...
        for op_ind in range(num_op_types):
            ...
            for i in range(new_out_label_ids.shape[0]):
                for j in range(new_out_label_ids.shape[1]):
...
def predict_batch_in_windows_TextTokenClassification_MultiLoss(
    ...
    returns = ave_aggregated_losses, aggregated_logits, aggregated_labels, None, batch_keys, originalword2subtokens
    #returns = ce_loss, np.concatenate(logits, axis=1), np.concatenate(labels, axis=1), deepcopy(batch['seg_boundaries'].detach().cpu().numpy()), batch_keys 
    for t in batch:
        del t
    return returns
