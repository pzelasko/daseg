...
class MTTransformer3ClassifLinearLayers(TrueCasingPunctuationTransformer):
    def __init__(self, target_label_encoder, model_name_or_path: str, inputs_embeds_dim: int, pre_trained_model: bool, max_sequence_length: int, label_smoothing_alpha=0, loss_weights=[1, 1]):
        super(pl.LightningModule, self).__init__()
        ...
        #import pdb; pdb.set_trace()    
        self.model = LongformerForTokenClassificationMT3ClassifLinearLayers(
            model_name_or_path,
            from_tf='.ckpt' in model_name_or_path,
            config=self.config)
...
class LongformerForTokenClassificationMT3ClassifLinearLayers(LongformerPreTrainedModel):
    def __init__(self, model_name_or_path, from_tf, config):
        super().__init__(config)
        ...
        self.classifier3 = nn.ModuleList([])
        for ind, label_count in enumerate(self.config.loss_hyperparameters['num_labels']):
            self.dropout.append(nn.Dropout(config.hidden_dropout_prob))
            self.classifier.append(nn.Linear(config.hidden_size, config.hidden_size))
            self.classifier2.append(nn.Linear(config.hidden_size, config.hidden_size))
            self.classifier3.append(nn.Linear(config.hidden_size, label_count))
    ...
    def forward(
        ...
        **kwargs
        ...
        for ind,label_count in enumerate(self.config.loss_hyperparameters['num_labels']):
            logits_perlabeltype = self.dropout[ind](sequence_output)
            logits_perlabeltype = self.classifier[ind](logits_perlabeltype)
            logits_perlabeltype = F.relu(logits_perlabeltype)
            logits_perlabeltype = self.classifier2[ind](logits_perlabeltype)
            logits_perlabeltype = F.relu(logits_perlabeltype)
            logits_perlabeltype = self.classifier3[ind](logits_perlabeltype)
...
class MTTransformer1Att1ClassifLinearLayers(TrueCasingPunctuationTransformer):
    def __init__(self, target_label_encoder, model_name_or_path: str, inputs_embeds_dim: int, pre_trained_model: bool, max_sequence_length: int, label_smoothing_alpha=0, loss_weights=[1, 1]):
        super(pl.LightningModule, self).__init__()
        ...
        #import pdb; pdb.set_trace()    
        self.model = LongformerForTokenClassification1Att1ClassifLinearLayers(
            model_name_or_path,
            from_tf='.ckpt' in model_name_or_path,
            config=self.config)
...
class LongformerForTokenClassification1Att1ClassifLinearLayers(LongformerPreTrainedModel):
    def __init__(self, model_name_or_path, from_tf, config):
        super().__init__(config)
        ...
        self.dropout = nn.ModuleList([])
        self.classifier = nn.ModuleList([])
        self.longformer_attlayers = nn.ModuleList([]) #LongformerLayer

        extra_config = deepcopy(self.config)
        extra_config.num_hidden_layers = 1
        for ind, label_count in enumerate(self.config.loss_hyperparameters['num_labels']):
            self.dropout.append(nn.Dropout(config.hidden_dropout_prob))
            self.classifier.append(nn.Linear(config.hidden_size, label_count))
            self.longformer_attlayers.append(LongformerEncoder(extra_config))
    ...
    def forward(
        ...
        **kwargs
        ...
        total_loss = []
        for ind,label_count in enumerate(self.config.loss_hyperparameters['num_labels']):
            logits_perlabeltype = self.longformer_attlayers[ind](hidden_states=sequence_output,
                                                            attention_mask=attention_mask)

            logits_perlabeltype = logits_perlabeltype[0]
            logits_perlabeltype = self.dropout[ind](logits_perlabeltype)
            logits_perlabeltype = self.classifier[ind](logits_perlabeltype)
