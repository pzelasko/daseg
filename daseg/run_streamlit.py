...
selected_act = st.sidebar.selectbox("Select dialog acts to explore:", options=label_set)

dialog_acts_with_context = find_dialog_acts(dataset, selected_act, left_context, right_context)

st.sidebar.header('Live examples')
example_index = st.sidebar.slider("Select which example to display:", 0, len(dialog_acts_with_context), 0)
# example_index = st.sidebar.selectbox("Select which example to display:", list(range(len(dialog_acts_with_context))))

selected_example = Call(dialog_acts_with_context[example_index])

st.header(f"Dialog Act Example: {selected_act}")
