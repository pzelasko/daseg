...
def dur2frame_map_tokentypeids(frame_len, seg_labels_path, features, target_label_encoder, segmentation_type='fine', padding_value_labels=-100):
    ...
    seg_boundaries = []
    token_type_ids = []
    frame_mismatch_buffer = int(0.5/frame_len) ## half a sec is allowed
    ## first get the features and corresponding labels in a plain style (A,A,A,A,H,H,N,N) with original seg_boundaries
    if '/' in seg_labels_path: # assuming a path includes '/'
        if os.path.isfile(seg_labels_path):
            ### for ERC task
                ...
                if seg_count_frames > 0:
                    ...
                    if (not emo_label is None) and (seg_count_frames > 0):
                        ...
                        if max(frame_indices) >= len(features):
                            ...
                            if max(frame_indices) - len(features) < frame_mismatch_buffer:
                                frame_indices = frame_indices[:-frame_mismatch_buffer]
                                seg_count_frames = len(frame_indices)
                            else:
                                raise ValueError(f'it seems max of frame_indices {max(frame_indices)} is a lot higher than length of features {len(features)} so exiting. Corresponding seg_labels_path is {seg_labels_path}')
                        new_features.append(features[frame_indices])
                        new_labels += [emo_label_original  for ind in range(seg_count_frames)]
                        ...
                        try:
                            ...
                            print(end_time, begin_time, frame_len, seg_count_frames, seg_labels_path)
                        seg_boundaries += temp
...
def concat_aug_ERC_SameEmotion(batch_data, batch_mask_data, batch_label, seg_boundaries, batch_token_type_ids):
    #### applicable only for isolated utts
    unique_labels = list(set(flatten(batch_label)))

    aug_ind = []
    for label in unique_labels:
        temp = [ind for ind,i in enumerate(batch_label) if list(set(i)) == [label]] # works because 
        if len(temp) > 0:
            temp_shuf = copy.deepcopy(temp)
            random.shuffle(temp_shuf) # for randomizing concatenating indices
            aug_ind.append(np.vstack([temp, temp_shuf]))
    ...
    batch_data_new = [np.concatenate([batch_data[ind1], batch_data[ind2]]) for ind1,ind2 in aug_ind]
    batch_mask_data_new = [np.concatenate([batch_mask_data[ind1], batch_mask_data[ind2]]) for ind1,ind2 in aug_ind]
    batch_label_new = [np.concatenate([batch_label[ind1], batch_label[ind2]]) for ind1,ind2 in aug_ind]
    seg_boundaries_new = [np.concatenate([seg_boundaries[ind1], seg_boundaries[ind2]]) for ind1,ind2 in aug_ind]
    batch_token_type_ids_new = [np.concatenate([batch_token_type_ids[ind1], batch_token_type_ids[ind2]]) for ind1,ind2 in aug_ind]
...
def collate_fn(batch, split = 'train', max_len=None, target_label_encoder=None, concat_aug=0.5, padding_value_features=0, padding_value_mask=0, padding_value_labels=0, frame_len=0.1, label_scheme='E', segmentation_type='smooth'):
    ...
    for seq in batch:
        if seq[3] > 0:
            ...
            utt_feat, utt_labels, utt_seg_boundaries, utt_toke_type_ids = dur2frame_map_tokentypeids(frame_len, seq[2], seq[1], target_label_encoder, segmentation_type, padding_value_labels=padding_value_labels)
    ...
    if (split == 'test') and ((len(batch) == 1) or (max_len is None)):
        ...
        if (ip_data_type == 'conversation') and (concat_aug == 1):
            ## concat_aug=1 if you want to remove context between all segments in a given conversation -- i.e., convert conversations into isolated utterances
        ...
        if ip_data_type == 'conversation':
            ...
            if (split == 'train') and (not concat_aug < 0) and (batch_size > 1):
                ...
                if concat_aug == 0:
                    ...
                    ## concatenate utt of same emotion. Make sure to provide higher batch size so that you will have enough samples of same emotion
                    while 1:
                        batch_data, batch_mask, batch_label, batch_seg_boundaries, batch_token_type_ids = concat_aug_ERC_SameEmotion(batch_data, batch_mask, batch_label, batch_seg_boundaries, batch_token_type_ids)
                        utt_lengths = [len(feat) for feat in batch_data]
                        no_times_concat += 1
                        if (min(utt_lengths) > int(max_len/2)) and (max(utt_lengths) > max_len):
                            break
...
def get_target_encoder_ERC(train_tsv, additional_labels=[], remove_labels=[], label_scheme=None):
    ...
    if isinstance(train_targets[0], float):
        return None
    else:
        train_labels = []
