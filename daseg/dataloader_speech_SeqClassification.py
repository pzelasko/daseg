...
def dur2frame_map_tokentypeids_SeqClassification(frame_len, seg_labels_path, features, target_label_encoder, segmentation_type='fine', padding_value_labels=-100):
    ...
    if '/' in seg_labels_path: # assuming a path includes '/'    
        ...
        if not target_label_encoder is None:
            ...
            emo_label = float(seg_labels_path)
        if (not emo_label is None)  and (seg_count_frames > 0):
            new_labels = emo_label
            seg_boundaries = [False for _ in range(seg_count_frames)]
            seg_boundaries[-1] = True
...
def to_batch_tensors(batch_data_new, batch_mask_data_new, batch_label_new, seg_boundaries_new, token_type_ids, 
    ...
    if isinstance(batch_label_new[0], list):
        # each sequence sample has a sequence of labels and classification for every frame
...
def perform_concat_aug(concat_aug, batch_label, target_label_encoder, batch_data_new):
    neutral_class_ind = target_label_encoder.transform(['neu'])[0] if 'neu' in target_label_encoder.classes_ else None

    utt2spkr = None
    aug_decision = np.random.choice(['NoConcatAug', 'ConcatAug'] , 1, p=[0.2, 0.8])  # 20% of the time original data and 80% of the time concatenated data
    if aug_decision == 'ConcatAug':
        # half the time concatenate same emotion utt and other half time concat neutral utt to emotional utt
        concattype_decision = np.random.choice(['SameEmotionConcat', 'NeutralConcat'], 1, p=[1-concat_aug, concat_aug])
        if concattype_decision == 'SameEmotionConcat':
            if not utt2spkr is None:
                aug_ind,batch_labels_aug,concat_portion = obtain_neutral_aug_ind(batch_label, batch_spk=batch_spk)
            else:
                aug_ind,batch_labels_aug,concat_portion = obtain_neutral_aug_ind(batch_label)

        elif concattype_decision == 'NeutralConcat':
            if neutral_class_ind is None:
                print(f'neutral_class_ind is None which could be because the supplied data_dir may not have "neu" class, please check.')
                print(f'Since neutral_class_ind is None and concattype is NeutralConcat, exiting...')
                sys.exit()
            if not utt2spkr is None:
                aug_ind,batch_labels_aug,concat_portion = obtain_neutral_aug_ind(batch_label, neutral_class_ind=neutral_class_ind, batch_spk=batch_spk)
            else:
                aug_ind,batch_labels_aug,concat_portion = obtain_neutral_aug_ind(batch_label, neutral_class_ind=neutral_class_ind)

        batch_data_new = obtain_batch_data_aug(aug_ind, batch_data_new, concat_portion)
        batch_label = batch_labels_aug
    return batch_data_new, batch_label


def collate_fn_SeqClassification(batch, split = 'train', max_len=None, target_label_encoder=None, concat_aug=0.5, padding_value_features=0, padding_value_mask=0, padding_value_labels=0, frame_len=0.1, label_scheme='E', segmentation_type='smooth'):
    ...
    for seq in batch:
        if seq[3] > 0:
            ...
            #import pdb; pdb.set_trace()
            try:
                utt_feat, utt_labels, utt_seg_boundaries, utt_toke_type_ids = dur2frame_map_tokentypeids_SeqClassification(frame_len, seq[2], seq[1], target_label_encoder, segmentation_type, padding_value_labels=padding_value_labels)
                ...
                batch_token_type_ids.append(utt_toke_type_ids)
                batch_length.append(seq[3])
            except:
                print(f'{seq[0]} has some problem with it, ignoring for training')
    ...
    if (split == 'test') and ((len(batch) == 1) or (max_len is None)):
        ...
        batch_data, batch_mask, batch_label, batch_seg_boundaries, batch_token_type_ids = cut_batch_to_maxlen(batch_data, batch_label,
                                                        batch_seg_boundaries, max_len, batch_token_type_ids=batch_token_type_ids, task_type='SeqClassif')
        
        if (split == 'train') and (not target_label_encoder is None) and (not concat_aug < 0):
    
            aug_decision = np.random.choice(['NoConcatAug', 'ConcatAug'] , 1, p=[0.2, 0.8]) # 20% of the time original data and 80% of the time concatenated data
            if aug_decision == 'ConcatAug':        
                batch_data, batch_mask, batch_label, batch_seg_boundaries, batch_token_type_ids = cut_batch_to_maxlen(batch_data, batch_label,
                                                            batch_seg_boundaries, int(max_len/2), batch_token_type_ids=batch_token_type_ids, task_type='SeqClassif')

                batch_data, batch_label = perform_concat_aug(concat_aug, batch_label, target_label_encoder, batch_data)
                ## temporary fixes for obtaining the following
                batch_mask = [[1]*len(i) for i in batch_data]
                batch_seg_boundaries = [[True]*len(i) for i in batch_data]
                batch_token_type_ids = [[0]*len(i) for i in batch_data]

        #if (split == 'train') and (not shuffle_ChunkLen_aug < 0):
        #    raise ValueError(f'Not yet fully implemented suffle augmentation')
        #    #batch_data_new = shuffle_chunks(batch_data_new, shuffle_ChunkLen_aug)
