...
def to_batch_tensors_MultiLabels(batch_data_new, batch_mask_data_new, batch_label_new, seg_boundaries_new, token_type_ids, 
                                            padding_value_features, padding_value_mask, padding_value_labels, split, batch_key, task_type='SeqTagging'):
    ''' Does padding to features, labels, mask, token_type_ids
    '''
    ## TODO: check where to pad either left or right
...
def cut_batch_to_maxlen_TrueCasingPunctuation(batch_data, batch_label, seg_boundaries, max_len, batch_mask=None, batch_token_type_ids=None, task_type='SeqTagging'):
    ''' Cuts the utterances longer than max_len to max_len. Shorter utts are kept as it is
    '''
    batch_data_new = []
    batch_mask_data_new = []
    ...
    for batch_ind,utt_feat in enumerate(batch_data):
        utt_labels = {}
        for label_name in batch_label.keys():
            utt_labels[label_name] = batch_label[label_name][batch_ind]
        ...
        if len(utt_feat) > max_len: # then randomly sample from the utterance
            ...
            if task_type == 'SeqClassif':
                ...
                for label_name in batch_label.keys():
                    utt_labels[label_name] = utt_labels[label_name][begin_index:begin_index+max_len]
                    utt_labels[label_name] = np.squeeze(np.vstack(utt_labels[label_name]))
        ...
        batch_mask_data_new.append(utt_mask)
        for label_name in batch_label.keys():
            if not label_name in batch_label_new:
                batch_label_new[label_name] = []
            batch_label_new[label_name].append(utt_labels[label_name])
        seg_boundaries_new.append(utt_seg_boundaries)
        batch_token_type_ids_new.append(utt_token_type_ids)
...
def obtain_text_fromcsv_TrueCasingPunctuation(tokenizer, transcript_path, seg_labels_path, target_label_encoder):
    '''This function is differey from obtain_text_fromcsv_TrueCasing. In this function, we assume the data dir
        already has labels and you need not detemine them here. Where as the function obtain_text_fromcsv_TrueCasing
        determines labels and hence it is not very flexible for diferent experiments
    '''
    ...
    transcript = pd.read_csv(transcript_path, sep=',')
    transcript.dropna(axis=0, inplace=True)
    transcript = transcript[['word', 'label', 'label2']]
    ...
    for word_label_pair in transcript:
        ...
        if word_label == 'DoNotExist':
            ...
            word_label = target_label_encoder['0'].transform([word_label])[0]
        ## computing loss only for first subtoken and discarding all other subtokens
        utt_labels['0'].append(word_label)
        if num_subwords > 1:
            utt_labels['0'] += [-100 for _ in range(num_subwords-1)]

        ## for label2
        if word_label2 == 'DoNotExist':
            word_label2 = -100
        else:
            word_label2 = target_label_encoder['1'].transform([word_label2])[0]
        
        ## not computing loss for all the subtokens except last subtoken
        if num_subwords > 1:
            utt_labels['1'] += [-100 for _ in range(num_subwords-1)]
        utt_labels['1'].append(word_label2)
        


    assert len(utt_labels['0']) == len(input_ids)
    assert len(utt_labels['1']) == len(input_ids)
    utt_seg_boundaries = [True]*len(input_ids)
    utt_token_type_ids = [0]*len(input_ids) # for BERT
    mask = [1]*len(input_ids)
    return input_ids, utt_labels, utt_seg_boundaries, utt_token_type_ids, mask
...
def obtain_text_fromcsv_TrueCasingPunctuation_Morethan2Tasks(tokenizer, transcript_path, seg_labels_path, target_label_encoder, split='train'):
    '''This function is differey from obtain_text_fromcsv_TrueCasing. In this function, we assume the data dir
        already has labels and you need not detemine them here. Where as the function obtain_text_fromcsv_TrueCasing
        determines labels and hence it is not very flexible for diferent experiments

        Also it is diff from obtain_text_fromcsv_TrueCasingPunctuation_v2. 
        It can deal with any number of tasks at the output where as prev version can only deal with 2 tasks in multitask setup
    '''
    ...
    for word_label_pair in transcript:
        ...
        original_word = word_label_pair[0]   
        word = word_label_pair[1]
        word_labels_alltasks = word_label_pair[2:]

        subword_tokens = tokenizer.tokenize(word)
        subword_tokens_ids = tokenizer.convert_tokens_to_ids(subword_tokens)
        input_ids += subword_tokens_ids
        num_subwords = len(subword_tokens_ids)

        if split == 'test':
            originalword2subtokens.append([original_word, subword_tokens])

        ## for label1
        for task_ind,word_label in enumerate(word_labels_alltasks):
            if word_label == 'DoNotExist':
                word_label = -100
            else:
                word_label = target_label_encoder[str(task_ind)].transform([word_label])[0]

            ## computing loss only for first subtoken and discarding all other subtokens
            utt_labels[str(task_ind)].append(word_label)
            if num_subwords > 1:
                utt_labels[str(task_ind)] += [-100 for _ in range(num_subwords-1)]
    ...
    utt_token_type_ids = [0]*len(input_ids) # for BERT
    mask = [1]*len(input_ids)
    return input_ids, utt_labels, utt_seg_boundaries, utt_token_type_ids, mask, originalword2subtokens


def obtain_text_fromcsv_TrueCasingPunctuation_Morethan2Tasks_MaxLen(tokenizer, transcript_path, seg_labels_path, target_label_encoder, split='train', max_len=512):
    '''This function is differey from obtain_text_fromcsv_TrueCasing. In this function, we assume the data dir
        already has labels and you need not detemine them here. Where as the function obtain_text_fromcsv_TrueCasing
        determines labels and hence it is not very flexible for diferent experiments
...
def get_target_encoder_TrueCasingPunctuation(train_tsv, additional_labels=[], remove_labels=[], label_scheme=None):
    ### it is an improvement to get_target_encoder

    ### assumes the labels for token/frame and this function can deal with multi-task model where you will have multiple labels
    train_targets = []
    train_data = pd.read_csv(train_tsv, sep=',', header=None)
    train_targets += list(train_data[1])
    ...
    if os.path.isfile(train_targets[0]):
        ...
        for utt_label_path in train_targets:
            temp = pd.read_csv(utt_label_path, sep=',')
            label_columns = [i for i in temp.columns if i.startswith('label')]
            label_columns = sorted(label_columns)

            label_block_names = {str(ind):label_name for ind,label_name in enumerate(label_columns)}

            for label_name,label_description in label_block_names.items():
                if label_name not in train_labels:
                    train_labels[label_name] = []            
                train_labels[label_name] += list(set(temp[label_description].values))
    ...
    label_enc = {}
    from sklearn import preprocessing
    for label_name in train_labels.keys():
        label_enc[label_name] = preprocessing.LabelEncoder()    
        temp_train_labels = train_labels[label_name]
    
        if label_scheme == 'Exact':
            temp_train_labels = temp_train_labels
        elif label_scheme == 'E':
            temp_train_labels += ['I-']
        elif label_scheme == 'IE':
            temp_train_labels += ['I-'+i for i in temp_train_labels]     
        else:
            raise ValueError('invalid label scheme')
       
        temp_train_labels += additional_labels  
        temp_train_labels = list(set(temp_train_labels))
        temp_train_labels = [i for i in temp_train_labels if not i in remove_labels if not pd.isna(i)]
...
def collate_fn_text_TrueCasingPunctuation(batch, split = 'train', max_len=None, target_label_encoder=None, concat_aug=0.5, padding_value_features=0, padding_value_mask=0, padding_value_labels=0, frame_len=0.1, label_scheme='E', segmentation_type='smooth', tokenizer=None):
    if isinstance(batch[0], tuple) and (len(batch[0]) == 1):
        batch = [batch[i][0] for i in range(len(batch))]
    ...
    for seq in batch:
        if seq[3] > 0:
            ...
            input_ids, utt_labels, utt_seg_boundaries, utt_token_type_ids, mask = obtain_text_fromcsv_TrueCasingPunctuation(tokenizer, seq[1], seq[2], target_label_encoder)
...
def collate_fn_text_TrueCasingPunctuation_Morethan2Tasks(batch, split = 'train', max_len=None, target_label_encoder=None, concat_aug=0.5, padding_value_features=0, padding_value_mask=0, padding_value_labels=0, frame_len=0.1, label_scheme='E', segmentation_type='smooth', tokenizer=None):
    if isinstance(batch[0], tuple) and (len(batch[0]) == 1):
        batch = [batch[i][0] for i in range(len(batch))]
