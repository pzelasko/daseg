...
# /dih4/dih4_2/jhu/Raghu/wiki_727/train/20/02/16/2002167
...
for call in data:
    ...
    for turn in call['turns']:
        for utt in turn['utterances']:
            ...
            doc_text += utt_text
            doc_labels += utt_topics_tokenwise

        if add_speaker_segmentation:
            doc_text += ['[SEP]']
            doc_labels += ['DoNotExist']


    if smooth_boundaries:
        doc_labels = obtain_smooth_boundaries(doc_labels)
    
    if only_boundary_label:
        doc_labels = remove_class_labels(doc_labels)
