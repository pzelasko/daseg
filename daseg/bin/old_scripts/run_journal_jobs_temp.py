...
QSUB_TEMPLATE = "qsub -l \"hostname=!c01*&c*,gpu={num_gpus},mem_free={mem}G,ram_free={mem}G\" -q {queue} -e {logerr} -o {logout} {script}"
...
submit(f"python /export/c02/rpapagari/daseg_erc/daseg/bin/dasg_temp train-transformer {opts[args.model_name]} -b {args.batch_size} -c 1 -e {args.no_epochs} "
       ...
       f"--monitor-metric-mode {args.monitor_metric_mode}   {outdir()} ", 
