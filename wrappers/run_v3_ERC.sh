...
if [[ $task_name == IEMOCAP_ConvClassif_xformer_smoothed_overlap_silence_OOS ]]; then
####################  convclassif with xformer and fs=10ms  ######################
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == IEMOCAP_ConvClassif_xformer_smoothed_overlap_silence_OOS_spkrind ]]; then
####################  convclassif with xformer and fs=10ms  ######################
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == IEMOCAP_ConvClassif_xformer_smoothed_overlap_silence_OOSNone_spkrind ]]; then
####################  convclassif with xformer and fs=10ms  ######################
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == IEMOCAP_ConvClassif_xformer_smoothed_overlap_silenceNone_OOSNone_NeutralNone_spkrind ]]; then
####################  convclassif with xformer and fs=10ms  ######################
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == IEMOCAP_ConvClassif_xformer_smoothed_overlap_silenceNone_OOSNone_NeutralNone ]]; then
####################  convclassif with xformer and fs=10ms  ######################
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == IEMOCAP_ConvClassif_xformer_smoothed_overlap_silence_OOSNone ]]; then
####################  convclassif with xformer and fs=10ms  ######################
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == IEMOCAP_ConvClassif_xformer_smoothed_overlap_NoSilence_OOS ]]; then
####################  convclassif with xformer and fs=10ms  ######################
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == IEMOCAP_ConvClassif_xformer_smoothed_overlap_NoSilence_AllClasses ]]; then
####################  convclassif with xformer and fs=10ms  ######################
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == ConvClassif_ResNet ]]; then
    data_dir=/export/b15/rpapagari/Tianzi_work/IEMOCAP_dataset/data_v2_ERC_NoInfreqEmo_ExciteMapHap_fs10ms_clean_noise123_music123/cv_${cv}
    no_epochs=100
    frame_len=0.01
    model_name=ResNet
...
######################   uttclassif   #############################
if [[ $task_name == UttClassif_longformer ]]; then
    corpus=IEMOCAP_v2
    max_sequence_length=512
    model_name=longformer
    pre_trained_model=False
...
if [[ $task_name == UttClassif_bilstm ]]; then
...
if [[ $task_name == UttClassif_ResNet ]]; then
    data_dir=/export/b15/rpapagari/Tianzi_work/IEMOCAP_dataset/data_v2_ASHNF_ExciteMapHap_fs10ms_clean_noise123_music123/cv_${cv}
    corpus=IEMOCAP_v2
    no_epochs=100
    frame_len=0.01
...
if [[ $task_name == SWBD_UttClassif_bilstm ]]; then
    corpus=SWBD_v2
    no_epochs=50
    max_sequence_length=512
    model_name=bilstm
    ...
    no_epochs=200
    suffix=_frame_len${frame_len}_UttClassif_bs${batch_size}_gacc${gacc}_concat_aug_${concat_aug}_Epochs_${no_epochs}
    test_file=test.tsv
    results_suffix=_with_uttids.pkl
fi

if [[ $task_name == SWBD_UttClassif_longformer ]]; then
...
if [[ $task_name == SWBD_UttClassif_ResNet ]]; then    
    corpus=SWBD_v2 #_CV_${cv}
    no_epochs=50
    max_sequence_length=2048
    model_name=ResNet
    ...
    no_epochs=200
    suffix=${pre_train_suffix}_frame_len${frame_len}_UttClassif_bs${batch_size}_gacc${gacc}_concat_aug_${concat_aug}_maxseqlen_${max_sequence_length}_Epochs_${no_epochs}
    test_file=test.tsv
    results_suffix=_with_uttids.pkl
fi

if [[ $task_name == SWBD_UttClassif_xformer ]]; then
    corpus=SWBD_v2 #_CV_${cv}
    no_epochs=50
    max_sequence_length=2048
    model_name=xformer
...
############################## bilstm ###
if [[ $task_name == SWBD_UttClassif_bilstm_seqclassif ]]
then
    corpus=SWBD_v2
    data_dir=/export/b15/rpapagari/Tianzi_work/SWBD/data_v2_isolated_utts_fs10ms/
    no_epochs=50
...
if [[ $task_name == SWBD_UttClassif_longformer_seqclassif ]]
then
    corpus=SWBD_v2
    data_dir=/export/b15/rpapagari/Tianzi_work/SWBD/data_v2_isolated_utts_fs10ms/
    no_epochs=50
    frame_len=0.01
    max_sequence_length=2048 # 4000  #2048
    test_file=test.tsv
...
if [[ $task_name == SWBD_ConvClassif_xformer_smoothed_overlap_silence_OOS ]]; then 
    # SWBD_ConvClassif
    corpus=SWBD_v2
    no_epochs=100
    frame_len=0.1
    ...
    suffix=${pre_train_suffix}_frame_len${frame_len}_ConvClassif_bs${batch_size}_gacc${gacc}_concat_aug_${concat_aug}_maxseqlen_${max_sequence_length}_warmup200steps_smoothed_overlap_silence_OOS
